...
struct MarketData {
    ...
    var title: String {
        ...
        return "\(marketInfo[1])/\(marketInfo[0])"
    }
    var price: String {
        switch NumberType(tradePrice) {
        case .int:
            return tradePrice.formatted()
        case .double1f:
            return String(format: "%.1f", tradePrice)
        case .double2f:
    ...
    var accPrice: String {
        ...
        if oneMillion <= accTradePrice24h {
            return Int(accTradePrice24h / oneMillion).formatted() + "백만"
        }
        
        return Int(accTradePrice24h).formatted()
    }
}

let testMockMarketData: [MarketData] = [
    MarketData(
        market: "KRW-BTC",
        tradeDate: "20250306",
        tradeTime: "053346",
